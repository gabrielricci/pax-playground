...
int mtd_scan_partitions(const char *mtdfile);
...
void mtd_write_set_limitation(MtdWriteContext *ctx, size_t pos);

/* MtdRegion is not opaque type (we need to have access to buffer) */
typedef struct MtdRegion {
    struct MtdReadContext *rd;
    struct MtdWriteContext *wr;
    char *buffer;
    off_t offset;
    size_t length;
} MtdRegion;

MtdRegion *mtd_grab_region(const MtdPartition *, off_t, size_t);
ssize_t mtd_commit_region(MtdRegion *);
